...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (termsModal) {
        termsModal.addEventListener('hidden.bs.modal', function () {
            // Opcional: Verificar si el usuario hizo clic en el botón "Acepto"
            // Esto requeriría agregar un event listener a ese botón
        });
        
        // Obtiene el botón de aceptar en el modal de términos
        const termsAcceptBtn = termsModal.querySelector('.modal-footer .btn-primary');
        if (termsAcceptBtn) {
    ...
    if (privacyModal) {
        privacyModal.addEventListener('hidden.bs.modal', function () {
            // Funcionalidad opcional después de cerrar el modal de privacidad
        });
        
        // Obtiene el botón de aceptar en el modal de privacidad
        const privacyAcceptBtn = privacyModal.querySelector('.modal-footer .btn-primary');
        if (privacyAcceptBtn) {
